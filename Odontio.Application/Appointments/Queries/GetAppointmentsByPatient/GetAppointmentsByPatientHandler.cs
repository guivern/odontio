...
public class GetAppointmentsByPatientHandler(IApplicationDbContext context) : IRequestHandler<GetAppointmentsByPatientQuery, PagedList<GetAppointmentResult>>
{
    public async Task<PagedList<GetAppointmentResult>> Handle(GetAppointmentsByPatientQuery request, CancellationToken cancellationToken)
    {
        var query = context.Appointments
            ...
            .Include(x => x.Patient)
            .Where(x => x.Patient.WorkspaceId == request.WorkspaceId && x.PatientId == request.PatientId)
